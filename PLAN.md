...
- [x] Deploy config prepared — `render.yaml`, `backend/Procfile`, env-var-driven CORS (`FRONTEND_ORIGIN`), full guide in `DEPLOYMENT.md`.
  ...
  - **Update**: user's real Render deploy attempts hit a genuine bug, in two rounds:
    1. `BAAI/bge-m3` via sentence-transformers (~2GB) OOM'd the free-tier 512MB web service outright.
    2. First fix attempt (swap to a smaller sentence-transformers model, `all-MiniLM-L6-v2`) looked right locally but **still OOM'd on Render** — new failure mode, no crash/log output for 3+ minutes then "Out of memory (used over 512Mi)". Diagnosed by measuring peak RSS locally (`/usr/bin/time -l`): sentence-transformers pulls in `torch`, and just importing torch uses ~600MB regardless of model size — already over budget before the app does anything.
    - **Real fix**: replaced `sentence-transformers` with `fastembed` (ONNX runtime, no torch) in `backend/ingest.py` and `backend/tools.py`, model `BAAI/bge-small-en-v1.5`. Measured peak RSS for a full real `/assess` call (model load + ChromaDB + LLM round trip): **~420MB**, fits with ~90MB headroom. Local ChromaDB rebuilt, retrieval spot-checked (distances actually tightened vs. before), full pytest suite green (48/48, and ~4x faster since torch is gone from the import chain). Documented in `DEPLOYMENT.md` with the measurement method so a future model/library swap can be re-checked before deploying.
    - Note: README's recorded DeepEval/benchmark numbers were measured against the original `bge-m3` embeddings and have not yet been re-run against `fastembed`'s embeddings — retrieval quality looks at least as good on spot-check, but not re-quantified with DeepEval yet.
