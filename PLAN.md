...
- [x] Deploy config prepared — `render.yaml`, `backend/Procfile`, env-var-driven CORS (`FRONTEND_ORIGIN`), full guide in `DEPLOYMENT.md`.
  ...
  - **Update**: user's first real Render deploy attempt hit a genuine bug — `BAAI/bge-m3` (the original embedding model) OOM'd the free-tier 512MB web service at runtime. Fixed by switching `EMBEDDING_MODEL_NAME` (`backend/ingest.py`) to `sentence-transformers/all-MiniLM-L6-v2` (~80MB vs. ~2GB); local ChromaDB rebuilt with the new model, retrieval spot-checked as still returning relevant chunks, full pytest suite still green. Documented in `DEPLOYMENT.md`. Note: README's recorded DeepEval/benchmark numbers were measured against `bge-m3` and have not yet been re-run against the new model.
